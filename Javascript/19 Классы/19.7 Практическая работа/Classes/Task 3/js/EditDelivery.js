...
import { deliveryCards, renderDeliveries } from "./index.js"
...
export default class EditDelivery extends Delivery {
	...
	getDialog() {
		...
		confirmButton.classList.add('btn', 'dialog__confirm-button')
	...
	static getTotalDistance(arr) {
		...
		arr.forEach(e => {
			if(e.status != 'canceled') {
				totalDistance += Number(e.distance)
			}
		})
	...
	static getTotalDistanceButton(arr) {
		const totalDistanceButton = document.createElement('button')
		totalDistanceButton.classList.add('btn', 'delivery__total-btn')
		totalDistanceButton.textContent = 'Общее расстояние'

		totalDistanceButton.addEventListener('click', () => {
			const totalDistance = EditDelivery.getTotalDistance(arr)
			
			let totalDistanceEl = document.querySelector('.delivery__total-distance-text')
			totalDistanceEl ? totalDistanceEl : totalDistanceEl = document.createElement('p')
			totalDistanceEl.classList.add('delivery__total-distance-text')
			totalDistanceEl.textContent = `Общее расстояние: ${totalDistance} км`

			totalDistanceButton.after(totalDistanceEl)
		})

		deliveryCards.after(totalDistanceButton)
	}
}